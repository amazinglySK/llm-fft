...
if [ "$#" -lt 3 ]; then
  echo "❌ Error: You must provide Weights & Biases entity, project name, and energy threshold."
    echo "Usage: $0 <wandb_entity> <wandb_project> <energy_threshold> [accelerator]"
  exit 1
fi
...
# Convert threshold to string suitable for filename (replace . with _)
THRESHOLD_STR=$(echo "$ENERGY_THRESHOLD" | tr '.' '_')
EXP_NAME="pretraining_lag_llama_filtered_threshold_${THRESHOLD_STR}"
...
do
    ...
    --wandb_entity "$WANDB_ENTITY" --wandb_project "$WANDB_PROJECT" --wandb_tags "lagllama" "filtered" "threshold_${ENERGY_THRESHOLD}" \
